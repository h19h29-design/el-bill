...
7. 업로드 직후 새로고침해도 `el-bill:storage-snapshot` 단일 루트에서 고지서·프로필·피크·요금표·파워플래너·출처가 함께 복원된다. 구버전 개별 키가 있는 브라우저에서는 가장 이른 만료 시각을 유지한 채 루트로 1회 마이그레이션되고, 성공 후 구버전 키가 제거된다.
8. 두 탭을 연 상태에서 한 탭이 새 파일을 올리면 다른 탭이 새 스냅샷을 통째로 채택한다. 오래된 탭의 일반 편집이 새 업로드를 덮어쓰지 않으며, 한 탭의 초기화 또는 정확한 24시간 만료가 다른 탭에도 샘플 복귀로 반영된다.
