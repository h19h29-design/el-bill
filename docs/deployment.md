...
7. 업로드 직후 새로고침해도 `el-bill:storage-active`가 가리키는 `el-bill:storage-snapshot:<sessionId>`에서 고지서·프로필·피크·요금표·파워플래너·출처가 함께 복원된다. 구버전 고정 루트 또는 개별 키가 있는 브라우저에서는 가장 이른 만료 시각을 유지한 채 세션 스냅샷을 먼저 저장하고 포인터를 마지막에 전환하며, 성공 후에만 구버전 키가 제거된다.
8. 두 탭을 연 상태에서 한 탭이 새 파일을 올리면 다른 탭이 포인터 승자의 스냅샷을 통째로 채택한다. 오래된 탭의 일반 편집·초기화·만료 정리가 새 업로드의 포인터 또는 스냅샷을 덮어쓰거나 삭제하지 않아야 하며, 활성 세션의 초기화 또는 정확한 24시간 만료는 다른 탭에도 샘플 복귀로 반영된다.
9. 브라우저 저장 용량 오류를 재현할 수 있는 검증 환경에서는 업로드가 사용자 업로드 상태로 표시되지 않고 한국어 저장 실패 안내가 보이며, 직전 정상 스냅샷이 그대로 복원되는지 확인한다.
