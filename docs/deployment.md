...
`npm run build`는 Vite 빌드 뒤 `npm run verify:bundle`을 자동 실행한다. 이 검사는 `dist/assets/index-*.js`의 가장 큰 초기 `index` 청크가 700,000 bytes를 넘거나 PDF.js 워커가 정적 서버에서 JavaScript MIME으로 제공하기 어려운 `.mjs`로 생성되면 실패한다. 문서 PDF 라이브러리와 대시보드·요금제 비교·피크·문서 생성 화면은 지연 로드되어 초기 진입 청크에 포함되지 않아야 한다. 지연 청크 로딩 자체가 실패하면 앱 셸과 사이드바는 유지하고, 뷰 영역에서 한국어 복구 패널과 페이지 새로고침 버튼을 제공해야 한다.
